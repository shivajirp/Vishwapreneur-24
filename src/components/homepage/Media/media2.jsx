...
const Media2 = () => {
  ...
  return (
    <div className='relative bg-black'>
      ...
      <div
        ref={countersRef}
        className='flex-col gap-y-36 p-10 md:flex-row lg:flex w-screen h-fit text-white place-content-around'
      >
        {socialIcons.map((icon) => (
          <motion.div
            key={icon.platform}
            className="m-8 border-2 border-r-2 border-white rounded-lg p-3 text-center h-fit place-content-center hover:transform hover:translate-y-[-5px] hover:transition-transform hover:ease-in-out hover:duration-400 duration-700 hover:border-r-2 hover:border-t-2 hover:border-[0px] hover:shadow-white hover:shadow-xl"
          >
            <i
              className={`${icon.icon} ion-3x `}
              onMouseOver={handleMouseOver}
            ...
            <span>
              {icon.platform === 'linkedin'
                ? 'Linkedin Fans'
                : `${icon.platform.charAt(0).toUpperCase() + icon.platform.slice(1)} Fans`}
            </span>
          </motion.div>
        ))}
      </div>
